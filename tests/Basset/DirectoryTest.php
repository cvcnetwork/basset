...
class DirectoryTest extends PHPUnit_Framework_TestCase {
  ...
  public function setUp()
  {
    ...
    $this->asset   = m::mock('Basset\Factory\AssetFactory', array(
      $this->files,
      $this->factory,
      'testing',
      'path/to/public'
  ...
  public function testAddingAssetFiresCallback()
  {
    ...
    $this->directory->javascript('foo.js', function () use (&$fired) {
      $fired = TRUE;
    });
  ...
  {
    $this->finder->shouldReceive('setWorkingDirectory')->once()->with('css')
      ->andThrow('Basset\Exceptions\DirectoryNotFoundException');
    $this->factory->shouldReceive('getLogger')->once()->andReturn(m::mock('Illuminate\Log\Writer')
      ->shouldReceive('error')->once()->getMock());
  ...
  public function testChangingWorkingDirectoryFiresCallback()
  {
    ...
    $fired = FALSE;
    $this->directory->directory('css', function () use (&$fired) {
      $fired = TRUE;
    });
    $this->assertTrue($fired);
  }
  ...
  public function testRequireDirectoryChangesDirectoryAndRequiresNewWorkingDirectory()
  {
    ...
    $requireDirectory->shouldReceive('iterateDirectory')->once()->with('foo/bar')
  ...
  public function testRequireCurrentWorkingDirectoryTree()
  {
    ...
    $directory->shouldReceive('recursivelyIterateDirectory')->once()->with('foo')
      ->andReturn($iterator = m::mock('Iterator'));
  ...
  public function testRequireTreeChangesWorkingDirectoryAndRequiresNewDirectoryTree()
  {
    ...
    $requireTree = m::mock('Basset\Directory[recursivelyIterateDirectory]', array(
      $this->factory,
      $this->finder,
      'foo/bar'
    ));
    ...
    $requireTree->shouldReceive('recursivelyIterateDirectory')->once()->with('foo/bar')
      ->andReturn($iterator = m::mock('Iterator'));
