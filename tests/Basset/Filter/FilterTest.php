...
class FilterTest extends PHPUnit_Framework_TestCase {
  ...
  public function setUp()
  {
    ...
    $this->filter = m::mock('Basset\Filter\Filter', array($this->log, 'FooFilter', array(), 'testing'))
      ->shouldDeferMissing();
  ...
  {
    $this->filter->shouldReceive('getClassName')->once()->andReturn('FilterWithConstructorStub');
    $this->filter->shouldReceive('getExecutableFinder')->once()
      ->andReturn(m::mock('Symfony\Component\Process\ExecutableFinder'));
  ...
  public function testFindingOfMissingConstructorArgsViaExecutableFinder()
  {
    $this->filter->shouldReceive('getClassName')->once()->andReturn('FilterWithConstructorStub');
    $this->filter->shouldReceive('getExecutableFinder')->once()
      ->andReturn($finder = m::mock('Symfony\Component\Process\ExecutableFinder'));
  ...
  {
    $filter = m::mock('Basset\Filter\Filter', array($this->log, 'FooFilter', array('path/to/node'), 'testing'))
      ->shouldDeferMissing();
    $filter->setResource($this->resource);
    $filter->shouldReceive('getClassName')->once()->andReturn('FilterWithConstructorStub');
    $filter->shouldReceive('getExecutableFinder')->once()
      ->andReturn($finder = m::mock('Symfony\Component\Process\ExecutableFinder'));
