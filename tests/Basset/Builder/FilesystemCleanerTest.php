...
class FilesystemCleanerTest extends PHPUnit_Framework_TestCase {
  ...
  public function testCleaningOfManifestFilesOnFilesystem()
  {
    ...
    $this->environment->shouldReceive('offsetGet')->once()->with('foo')
      ->andReturn($collection = m::mock('Basset\Collection'));
    ...
    $this->files->shouldReceive('delete')->once()->with('path/to/builds/foo-asfjkb8912h498hacn8casc8h8942102.css')
      ->andReturn(TRUE);
    ...
    $entry->shouldReceive('getDevelopmentAssets')->once()->andReturn(array(
      'stylesheets' => array(
        'bar/baz-37b51d194a7513e45b56f6524f2d51f2.css',
        'bar/qux-acbd18db4cc2f85cedef654fccc4a4d8.css'
      ),
      'javascripts' => array()
    ));

    $this->files->shouldReceive('glob')->once()->with('path/to/builds/foo/bar/baz-*.css')
      ->andReturn(array('path/to/builds/foo/bar/baz-37b51d194a7513e45b56f6524f2d51f2.css'));
