...
class AssetFinderTest extends PHPUnit_Framework_TestCase {
  ...
  public function testFindRemotelyHostedAsset()
  {
    $this->config->shouldReceive('get')->once()
      ->with('basset::aliases.assets.http://foo.bar/baz.css', 'http://foo.bar/baz.css')
      ->andReturn('http://foo.bar/baz.css');
  ...
  {
    $this->config->shouldReceive('get')->once()->with('basset::aliases.assets.//foo.bar/baz.css', '//foo.bar/baz.css')
      ->andReturn('//foo.bar/baz.css');
  ...
  public function testFindPackageAsset()
  {
    ...
    $this->config->shouldReceive('get')->once()->with('basset::aliases.assets.bar::baz.css', 'bar::baz.css')
      ->andReturn('bar::baz.css');
  ...
  {
    $this->config->shouldReceive('get')->once()->with('basset::aliases.assets.foo.css', 'foo.css')
      ->andReturn('foo.css');
  ...
  {
    $this->config->shouldReceive('get')->once()
      ->with('basset::aliases.assets./absolute/path/to/foo.css', '/absolute/path/to/foo.css')
      ->andReturn('/absolute/path/to/foo.css');
  ...
  {
    $this->files->shouldReceive('exists')->once()->with('path/to/public/working/directory')->andReturn(TRUE);
    $this->files->shouldReceive('exists')->once()->with('path/to/public/working/directory/foo/bar/baz')
      ->andReturn(TRUE);
